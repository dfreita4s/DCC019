...
mata(marcellus, X) :- massagem(mia, X).
ama(mia, X) :- bonsDancarinos(X).
come(jules, X) :- nutritiva(X) ; saborosa(X).
...
% Faça:
%%%%%
%As respostas estao comentadas para nao sujar o cache 
%%%%%
...
% c) um predicado pai, de dois argumentos, de modo que pai(X, Y) represente o fato de que X é pai de Y;
  pai(X, Y) :- filho(Y, X), X \= Y.
% d) uma consulta, usando o predicado pai, para determinar quem é o pai de tiago; pai(X, tiago). 
% e) um predicado tio/2, de maneira que tio(X, Y) expresse que X é tio de Y (desconsidere o sexo);
  ...
  tio(X, Y) :-  filho(Y, Z), irmao(X, Z), X \= Y.
% f) um predicado primo/2, de maneira que primo(X, Y) represente o fato de que X é primo de Y;
  primo(X, Y) :- pai(Z, Y), tio(Z, X), X \= Y.
% g) um predicado parente/2, de modo que parente(X, Y) represente o fato que X é parente de Y.
  sobrinho(X, Y) :- tio(Y, X), X \= Y.
  avo(X, Y) :- pai(X, Z), pai(Z, Y), X \= Y.
  neto(X, Y) :- avo(Y, X), X \= Y.
  parente(X, Y) :- filho(X, Y); pai(X, Y) ; avo(X, Y) ; neto(X, Y) ; irmao(X, Y) ; tio(X, Y); sobrinho(X, Y) ; primo(X, Y), X \= Y.
